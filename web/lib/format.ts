 ...
 * ".00" (the common whole-yen case) while preserving any 端数. Negatives follow the accounting
 * convention 簿記/決算書 use: a leading 三角 △ replaces the minus sign (and the ¥), so a 赤字 /
 * 控除 reads as "△580,500" rather than "-¥580,500" (see the 朱 tint in {@link Amount}). The raw
 * string stays the source of truth — this only affects presentation.
 */

/** Format a fixed-point amount string as a grouped figure ("¥300,000", negatives as "△350,000"). */
