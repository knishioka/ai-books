...
./scripts/verify.sh --web     # + web lint/typecheck/unit test
```

- 内部で lint / format / typecheck / e-Tax layout sync check / test を順に実行する (build は library なので `n/a`)。
- `--web` 指定時は追加で `web/` の lint / typecheck / unit test を実行する。
...
./scripts/test.sh --e2e    # supabase start + Playwright で viewer のブラウザスモーク (#162)
...
| Web Vercel parity build (isolated web root)      | `web/` 単独 Root で本番 build (`../src` 等の repo-root 参照を検出)                                 | #140                |
| e-Tax layout sync check                          | Python 側 layout SSOT と web 生成コピーの drift を検出                                             | #154                |
...
| Viewer E2E smoke + fail-closed auth (Playwright) | 全 10 画面が描画され、未認証/allowlist 外は `/login` に fail-closed (`supabase start` で実 GoTrue) | #162                |
...
lint / format / typecheck / e-Tax layout sync check は `./scripts/verify.sh`(= CI `pre-commit` 相当の静的検査)で担保する。
...
- **P1 — 検証の通過**: `./scripts/verify.sh` (lint/format/typecheck/e-Tax layout sync/test) が green か。DB 連携を含む変更は
