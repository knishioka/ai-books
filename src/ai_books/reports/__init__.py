"""帳簿・決算書レポートの出力フォーマット (Issue #19 帳簿 / #20 損益計算書).

The report *data* — 仕訳帳 (:class:`~ai_books.models.JournalBook`), 総勘定元帳
(:class:`~ai_books.models.GeneralLedger`) and 損益計算書 (:class:`~ai_books.models.ProfitAndLoss`)
...
* **機械可読** — :func:`journal_book_snapshot` / :func:`general_ledger_snapshot` /
  :func:`profit_and_loss_snapshot` (canonical JSON dicts, the shape the golden harness freezes
  and the Vercel viewer #25 renders) and :func:`journal_book_csv` / :func:`general_ledger_csv`
  (flat CSV for the tabular 帳簿).
* **人間可読** — :func:`journal_book_text` / :func:`general_ledger_text` /
  :func:`profit_and_loss_text` (整形テキスト for eyeballing).
