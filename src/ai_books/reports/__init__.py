"""帳簿・決算書レポートの出力フォーマット (Issue #19 帳簿 / #21 貸借対照表).

The report *data* — 仕訳帳 (:class:`~ai_books.models.JournalBook`), 総勘定元帳
(:class:`~ai_books.models.GeneralLedger`) and 貸借対照表 (:class:`~ai_books.models.BalanceSheet`)
...
* **機械可読** — :func:`journal_book_snapshot` / :func:`general_ledger_snapshot` /
  :func:`balance_sheet_snapshot` (canonical JSON dicts, the shape the golden harness freezes and
  the Vercel viewer #25 renders) and :func:`journal_book_csv` / :func:`general_ledger_csv`
  (flat CSV).
* **人間可読** — :func:`journal_book_text` / :func:`general_ledger_text` /
  :func:`balance_sheet_text` (整形テキスト for eyeballing).
