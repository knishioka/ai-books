...
3. [docs/README.md](./docs/README.md) — ドキュメントハブ (全文書の索引と SSOT マップ)
4. [.github/PULL_REQUEST_TEMPLATE.md](./.github/PULL_REQUEST_TEMPLATE.md) — PR 本文の型
...
**`./scripts/test.sh --all` が唯一の『全部動く』確認** (#59)。各モードの内訳・CI との保証範囲
対応は [AGENTS.md#verification](./AGENTS.md#verification) を正とする (ここでは重複させない)。
...
## エージェント権限・自動化 (hooks / guards)

AI エージェントの権限 (allow/deny) と **hooks** は **[.claude/settings.json](./.claude/settings.json) が SSOT**。
個人差分は gitignore 済の `.claude/settings.local.json` に置く。詳細は [AGENTS.md#agent-permissions](./AGENTS.md#agent-permissions)。

- **編集ガード/自動整形**: PreToolUse が「Never touch」(applied migration / `.env` / `*.xsd` /
  `uv.lock` / `~/.ai-books/`) の編集を自動ブロックし、PostToolUse が `ruff format` を自動適用する。
  挙動・回避手順は [docs/ai/hooks-and-guardrails.md](./docs/ai/hooks-and-guardrails.md)。
- **レビュー観点**: PR レビューの観点 (Claude / Codex 共通) は [AGENTS.md#review-guidelines](./AGENTS.md#review-guidelines)。
