...
**`./scripts/test.sh --all` が唯一の『全部動く』確認** (#59)。Postgres + pgbouncer を 1 回
立ち上げ、Python 全テスト (MCP #50/#56・property #57・read-only ロール #54 含む) + カバレッジ
ゲート (#58)、pooler 安全性 + golden (pgbouncer 越し #52)、web ユニット + カバレッジ (#55/#58)、
viewer golden (直結) を**まとめて 1 回**で走らせ、最後に **PASS/FAIL サマリ**を出す。途中で
止めず全ブロックを実行し、1 つでも失敗すれば非ゼロ終了する。CI との保証範囲対応は
[README#ci--local-guarantee-mapping](./README.md) / [AGENTS.md#verification](./AGENTS.md#verification) 参照。
