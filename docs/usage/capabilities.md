...
1. `export_etax(fiscal_year="FY2025", format="xtx")` で実 e-Tax 交換ファイル `.xtx`
   (官式 KOA210 一般用) を生成。`csv` / `xml` は人間確認用の補助で e-Tax には取り込めない。
2. 必須項目・桁数・科目コード・月のスキーマ検証に通らないと `ToolError` が全件まとめて返る
   → 帳簿側を直して再生成。
3. **取込 → 署名 → 送信の手順**は [docs/etax/handoff-runbook.md](../etax/handoff-runbook.md)。
