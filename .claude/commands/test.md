...
argument-hint: "[--web] [--pooler] [--all] [--down] [pytest args...]"
...
- `--pooler` runs the suite through a pgbouncer transaction pooler (Supabase parity, #52).
- `--all` is the one-command local guarantee (every block + PASS/FAIL summary, #59); see `/test-all`.
